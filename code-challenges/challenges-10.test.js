...
const createServer = () => {
  ...
  app.get('/aboutme', (req, res) => { res.send('I am a dev') });
  app.get('/favoritefoods', (req, res) => { res.send(['bananas', 'carrots']) });
...
const count = (target, input) => {
  ...
  input.map(element => {
    ...
    let numArray = element.filter(num => num === target)
...
const divisibleByFiveTwoToThePower = (input) => {
  let arrays = input.map(elemental => {
    ...
    let square = filterFive.map(element => Math.pow(2, element));
    return square;
  });
  return arrays;
};
