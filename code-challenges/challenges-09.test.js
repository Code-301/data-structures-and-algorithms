...
const mapCurrentEvents = (() => {
  ...
  let newsStories = [];
  currentEvents.news.map(data => {
    newsStories.push(new Event(data));
...
const countNumberOfElements = (arr) => {
  let newElem = arr.reduce((acc, elem) => {
    acc++;
    return acc;
  }, []);
  return newElem;
...
const returnNames = (arr) => {
  let namesList = arr.reduce((nam, element) => {
    nam.push(element.name);
    return nam;
  }, []);
  return namesList;// Solution code here...
};
...
const reversedString = (str) => {
  ...
  let reverserMan = strSplitter.reduce((revStr, element) => {
    return element + revStr;
  }, '');
  return reverserMan;
};
