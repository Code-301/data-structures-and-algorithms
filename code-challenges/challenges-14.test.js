...
const createServer = () => {
  ...
  app.get('/', (req, res) => res.status(200).send(200));
  app.delete('/things/1', (req, res) => res.status(405).send(405));
...
let biggerThanLuke = (arr) => {
  if (arr.length > 0) {
    let massTest = arr[0].mass;
    let finalString = '';
    arr.map((data, idx) => {
      // console.log('dataMass', data.mass, 'massTest', massTest);
      if (parseInt(data.mass) > parseInt(massTest)) {
        finalString += data.name;
        if (idx < arr.length - 1) {
          finalString += ' - ';
        }
      }
